...
import { URL, datas } from './chartUtils.js';
...
function setCharts(lang) {
  fetch(URL)
    ...
    .then((data) => {
      ...
      // for (let key in datas) datas[key].forEach((arr) => (arr = []));
      const container = {};

      for (let key in datas) datas[key].forEach((arr) => (container[arr] = []));

      if (lang === 'py') {
        for (const key in data['py']['problem_type_count']) {
          container.problemTypeCountData.push(
            data['py']['problem_type_count'][key]
          );
        }
        ...
        const problemTypeCountChart = new Chart(
          document.getElementById('problem-type-chart-1'),
          {
            ...
            data: {
              ...
              datasets: [
                {
                  data: container.problemTypeCountData,
                  backgroundColor: [
                    'rgba(255, 99, 132, 0.2)',
                    'rgba(54, 162, 235, 0.2)',
                    'rgba(255, 206, 86, 0.2)',
        ...
        for (const key in data['js']['problem_type_count']) {
          container.problemTypeCountData.push(
            data['js']['problem_type_count'][key]
          );
        }
