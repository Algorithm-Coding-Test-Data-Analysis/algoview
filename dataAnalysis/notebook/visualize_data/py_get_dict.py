...
def get_ptype_dict(df):
    ...
    # 문제유형별 내장함수,메서드 사용빈도
    temp_df = drop_duplicated_solution(df)
    ...
    # 문제유형별 내장함수 사용빈도
    temp_df = drop_duplicated_solution(df)
    temp_df = temp_df.loc[temp_df['function_method'].str.isalpha()] # module 및 메서드( .(dot)으로 시작하는 형태 제외)
    temp_df = temp_df.groupby(['problem_type', 'function_method'])['countmethod'].sum()
    temp_df = temp_df.groupby('problem_type').nlargest(top_n).reset_index(level = 0, drop = True)
    ...
    # 문제유형별 메서드 사용빈도
    temp_df = drop_duplicated_solution(df)
    temp_df = temp_df.loc[temp_df['function_method'].str.startswith('.')]
    temp_df = temp_df.groupby(['problem_type', 'function_method'])['countmethod'].sum()
    temp_df = temp_df.groupby('problem_type').nlargest(top_n).reset_index(level = 0, drop = True)
